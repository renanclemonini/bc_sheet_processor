...
async def upload_excel(file: UploadFile = File(...)):
    ...
    if not file.filename.endswith(('.xlsx', '.xls')):
        raise HTTPException(400, detail="Apenas arquivos Excel (.xlsx, .xls) são aceitos")
...
async def download_arquivo(job_id: str):
    ...
    if job.get("status") != "completed":
        ...
        raise HTTPException(400, detail=f"Processamento ainda não finalizado. Status: {job.get('status')}")
    ...
    try:
        ...
        with open(arquivo_entrada, 'rb') as f:
            ...
            if not padrao_3_colunas and not padrao_4_colunas:
                raise ValueError("Formato de planilha não reconhecido. Colunas necessárias não encontradas.")
            ...
            for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True)):
                ...
                for col in ['etiquetas', 'etiqueta', 'tag']:
                    if col in idx and len(row) > idx[col]:
                        ...
                        etiquetas = f'{val}, {etiqueta_padrao}' if val and val.lower() != 'nan' else etiqueta_padrao
                        break
        ...
        print(f"[{job_id}] Arquivo original: {linhas_originais} linhas x {colunas_originais} colunas")
